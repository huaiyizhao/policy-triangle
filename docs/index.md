...
Classical policy gradients begin with REINFORCE; TRPO and PPO add proximal
control so that sampled data can be reused without arbitrarily large updates.
PPO and group-relative variants remain common in LLM training, while
REINFORCE-style objectives such as ReMax, RLOO, CISPO, and TOPR have renewed
interest.

At system scale, inference and training are usually separate. A rollout may be
generated by stale weights, a different numerical backend, lower precision, or
different mixture-of-experts routing. Thus the probability recorded by the
rollout engine need not equal the probability recomputed by the learner, even
when both nominally refer to the same checkpoint. Several optimizer updates can
further move the trainable policy before the sample is consumed.

Recent methods address these effects with a rapidly expanding vocabulary:
decoupled PPO, TIS and MIS, IcePop and KPop, GSPO, DPPO, TRM, ESTR, and A-3PO.
The mechanisms are often presented as unrelated losses, obscuring whether an
operation selects data, changes measure, or constrains the current update.
...
sign-dependent bounds; GSPO replaces the token ratio with a length-normalized
sequence-geometric ratio.
...
Self-normalization is orthogonal:
...
|---|---|---|---|
| PPO / GRPO / DAPO | Coupled | \\(\mathrm C_{\mathrm{tok}}(E)\\) | One direct ratio carries correction and proximal-control roles; DAPO uses asymmetric bounds. |
| CISPO / TOPR | Direct PG | \\(\mathrm W_{\mathrm{tok}}(E)\\) | Detached clipped or tapered weight; gradients need not vanish outside a PPO band. |
| GSPO | Coupled | \\(\mathrm C_{\mathrm{geo}}(E)\\) | Sequence-geometric shaping; the original objective does not add a separate pre-filter. |
| Decoupled PPO / AReaL / A-3PO | Factorized | \\(\mathrm W(B)\mathrm C(U)\\) | Behavior correction is detached from the trainable proximal constraint. |
| TIS / MIS / rollout correction | Either | \\(\mathrm M(B\text{ or }E)\mathrm W(B\text{ or }E)\\), optional \\(\mathrm C\\) | Weighting and rejection compose; bypass and decoupled modes change edge semantics. |
| IcePop / KPop | Usually direct | \\(\mathrm M_{\mathrm{tok}}(E)\\) plus a base loss | IcePop uses a ratio region; KPop uses bidirectional binary-KL geometry sensitive to absolute probability. |
| DPPO / TRM | Direct | Divergence \\(\mathrm M(E)\\) or \\(\mathrm C(E)\\) | Distributional geometry replaces sampled-ratio magnitude; TRM rejects a sequence on worst-token divergence. |
...
Decoupled PPO is the closest precursor because it explicitly inserts a proximal
policy between behavior and current policies. AReaL and A-3PO operationalize or
approximate that factorization for asynchronous LLM training. The triangle is
broader in operator placement but does not prescribe a particular proximal
anchor.

The veRL rollout-correction formulation and FP8-RL compare practical TIS, MIS,
...
RPG and the off-policy interpretation of group-relative REINFORCE give deeper
analyses of KL regularization, baselines, and data shaping. Those axes are
intentionally outside this critic-free policy-loss taxonomy. Jackpot also lies
partly outside the view because it changes the behavior distribution through
rejection and joint rollout-model updates, rather than only operating on stored
trajectories downstream.
